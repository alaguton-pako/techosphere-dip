...
<template>
  <section class="my-12 flex flex-row-reverse items-center gap-6">
    ...
    <div class="flex items-center">
      ...
      />
      <button
        class="h-full rounded-r-md border border-secondary bg-primary-gray py-2.5 px-3"
      >
...
<thead class="bg-gray-200 text-base ">
  <tr class="border border-secondary">
    ...
    <th class="border border-secondary p-3">Last Date Updated</th>
...
<tbody v-if="requestCompleted && allNetworkFaults != null">
  <tr v-for="(item, index) in allNetworkFaults" :key="index">
    ...
    <td class="border border-secondary p-4">
      <router-link
        :to="{ name: 'obligationE', params: { id: item.id } }"
