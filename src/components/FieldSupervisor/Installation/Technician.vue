  ...
  <table class="mt-1 w-full table-auto border border-secondary text-sm">
    <thead class="bg-gray-200">
      <tr class="capitalize">
        ...
        <th class="p-3">Total Activity</th>
    ...
    <tbody>
      <tr v-for="(installer, index) in installers" :key="index">
        <td class="border border-secondary p-3">{{ installer.name }}</td>
        <td class="border border-secondary p-3 text-center">
          {{ installer.totalActivity }}
        </td>
        <td class="border border-secondary p-3 text-center">{{ installer.achieved }}</td>
        <td class="border border-secondary p-3 text-center">
          {{ ((installer.achieved / installer.totalActivity) * 100).toFixed(2) }}%
        </td>
      </tr>
    </tbody>
...
const totalActivity = computed(() =>
  installers.value.reduce((total, installer) => total + installer.totalActivity, 0)
);

const totalAchieved = computed(() =>
  installers.value.reduce((total, installer) => total + installer.achieved, 0)
);
