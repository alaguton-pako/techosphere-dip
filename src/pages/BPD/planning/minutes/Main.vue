...
import Table from "../meeting/Table.vue";
...
<template>
  <div>
    ...
    <section class="flex flex-row py-10 px-10">
      <Card v-for="(card, index) in cardsData" :key="index" :figure="card.figure" :title="card.title"
        :bgColor="card.bgColor" :textColor="card.textColor" :body="card.body" heigh="h-48" width="w-80" />
    </section>
    <section>

      <Table :activeTab="props.activeTab" @setActiveMeetingId="setActiveMeetingId"> </Table>
    </section>
  </div>
</template>
