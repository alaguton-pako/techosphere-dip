...
const submitPerformanceRating = async () => {
  try {
    const payload = {
      objectives: performanceRating.value.performance_rating.map(
        (performance) => {
          let rating
          if (performance.kpi === 'Process Compliance') {
            rating = compliance.value
          } else if (performance.kpi === 'Process Efficiency') {
  ...
  <app-layout>
    <template v-slot:main-content>
      <main class="w-full bg-[#F5F5F5] pl-[260px]">
        ...
        <!-- <section class="p-9"><NameTag /></section> -->
        <section class="p-10">
          <div @click="router.back()" class="pr-4">Back</div>
          <section>
