  ...
  <section>
    <div class="flex w-[770px] justify-between bg-white">
      <div class="rounded-md p-8">
        ...
        <div class="mb-4 flex space-x-3">
          ...
          <span class="italic text-primary">Click here to redirect to Academy</span>
        </div>
        ...
        <ol class="mx-6 list-decimal space-y-2 grid grid-cols-2 space-x-4">
    ...
    <section class="" v-if="activeTab == 1">
      <div class="w-[770px] justify-between rounded-md bg-white p-5">
        ...
        <section class="grid grid-cols-4 gap-2 font-[12px]">
          ...
          <div class="rounded-md border border-secondary-gray bg-white text-center">
            ...
            <div class="py-10 text-[32px] font-bold">
              {{ myPerformance.daily_schedule }}/{{ myPerformance.working_days }}
            </div>
          </div>
          <div class="rounded-md border border-secondary-gray bg-white text-center">
            <h2 class="border-b border-b-secondary-gray py-4 text-sm font-bold">
            ...
            <div class="py-10 text-[32px] font-bold">
              {{ myPerformance.closeout_report }}/{{ myPerformance.working_days }}
            </div>
          </div>
          <div class="rounded-md border border-secondary-gray bg-white text-center">
            <h2 class="border-b border-b-secondary-gray py-4 text-sm font-bold">
        ...
        <section>
          <p class="mt-8 mb-4 text-xl font-bold">My KPIs</p>
          <table
            class="w-full table-auto border-secondary text-center text-sm font-normal"
          >
            <thead class="bg-gray-200 p-2">
              <tr class="border-secondary">
                ...
                <th class="border-r border-secondary p-3 font-normal">MATRIX/MEASURES</th>
                <th class="border-secondary p-3 font-normal">TARGET</th>
                <th class="border-secondary p-3 font-normal">WEIGHING (%)</th>
                <th class="border-secondary p-3 font-normal">MAX RATING OBTAINABLE</th>
              </tr>
            </thead>
            <tbody>
              <tr
          ...
          >
            ...
            >
              ...
              <div
                ...
                :class="{
                  'border-green-600 bg-green-600': request.status === 'approved',
            ...
            <div :data-reqeust="`${request.name}-${index}`" class="hidden p-6">
              <h2 class="my-3"><b> Start date:</b> {{ request?.start_date }}</h2>
              ...
              <h2 class="my-3"><b> Description:</b> {{ request?.description }}</h2>
            </div>
          </section>
        </section>
      </div>
...
onMounted(() => {
  // Promise.all(
  getMyRequest(), getMyRequestSummary(), getMyKPI(), getUserProfile(), getMyPerformance();
  // );
});
...
//   try {
//     const { status, data } = await workplaceRequestsv2("get", "leave/my-leave-requests");
//     if (status === 200) {
...
//   try {
//     const { status, data } = await workplaceRequestsv2("get", "leave/user/summary");
...
const getMyPerformance = async () => {
  try {
    const { status, data } = await workplaceRequestsv2("get", "user/performance");
    if (status === 200) {
